...
const ctrl = __importStar(require("../controllers/admin.controller"));
...
router.get("/vendors/registrations", async (_req, res) => {
    try {
...
router.get("/logs", async (req, res) => {
    try {
        const q = req.query;
        const logs = await ctrl.listActivityLogs({ userId: q.userId, adminUserId: q.adminUserId }, q.limit ? parseInt(q.limit) : 100);
        res.json(logs);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
