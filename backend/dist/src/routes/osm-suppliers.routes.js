...
function preferredCategories(query) {
    const q = query.trim().toLowerCase();
    if (!q)
        return [];
...
function filterByItemQueryOrFallback(rows, query) {
    const q = query.trim().toLowerCase();
    if (!q)
        return rows;
...
function buildOverpassQuery(filters, radius, lat, lng) {
    return [
        "[out:json][timeout:18];",
        "(",
...
async function callOverpass(query, endpoint) {
        ...
        for (const step of queryPlan) {
            for (const radius of step.radii) {
                ...
                mappedSuppliers = (payload.elements ?? [])
                    ...
                    if (typeof latitudeRaw !== "number" || typeof longitudeRaw !== "number")
                        return null;
