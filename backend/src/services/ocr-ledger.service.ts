...
export type OcrLedgerStructured = {
  items: OcrItem[];
  expenses: OcrExpense[];
...
function textNoisePenalty(text: string): number {
  const garbledMarkers = (text.match(/[?]{2,}|\uFFFD|[|]{2,}|[_]{2,}/g) || []).length;
  if (garbledMarkers >= 6) return 0.35;
  if (garbledMarkers >= 3) return 0.2;
  if (garbledMarkers >= 1) return 0.1;
  return 0;
}

function characterErrorPenalty(text: string): number {
  const confusionPatterns = (text.match(/\b\d+[oO]\b|\b[oO]\d+\b|\b[a-zA-Z]*\d[a-zA-Z]*\b/g) || []).length;
  if (confusionPatterns >= 6) return 0.28;
  if (confusionPatterns >= 3) return 0.16;
  if (confusionPatterns >= 1) return 0.08;
  return 0;
}

function layoutScore(text: string): number {
  const lines = text.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  if (!lines.length) return 0.35;
...
function inferencePenalty(notes: string[]): number {
...
function numericSignal(text: string): number {
  ...
  if (numericTokens.length >= 3) return 1;
...
export async function extractTextFromImage(imageBuffer: Buffer): Promise<{ text: string; ocrEngineConfidence: number }> {
...
export async function extractLedgerFromOcrText(rawText: string, ocrEngineConfidence: number): Promise<OcrLedgerStructured> {
  ...
  const items: OcrItem[] = itemsRaw
    ...
    .filter((v): v is OcrItem => v !== null);

  const expenses: OcrExpense[] = expensesRaw
    .map((v) => {
      const row = v as Record<string, unknown>;
  ...
  const scoredItems = items.map((item) => {
    ...
    const ambiguity = Math.max(0, Math.min(1,
      (1 - qualityScore) * 0.45 +
